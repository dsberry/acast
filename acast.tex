...
be serialised in various ways. The astWrite method of the basic astChannel
...
``sink'' function can be supplied to the astChannel constructor which --- if
supplied --- will be called by the astChannel class to store each text
string in some associated external store. The astRead method of the
astChannel class does the inverse --- it calls a supplied ``source''
...
AST objects produced by the astChannel class use a bespoke block structured
format specific to AST.

Within the Python interface, the \emph{print} function uses an astChannel
...
Attributes of the astChannel class can be used to enable or disable
...
support 3-D coordinate grid plotting in the GAIA application using VTK
\citep{2008ASPC..394..339D}.
...
source code for the AST library is open-source using the Lesser Gnu
